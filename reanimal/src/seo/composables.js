...
export function useSEO() {
  ...
  const updateMetaTags = () => {
    ...
    updateMetaTag('og:site_name', 'Reanimal Game Community', 'property')
  ...
  const generateStructuredData = (pageType = 'WebPage') => {
    ...
    if (pageType === 'VideoGame') {
      ...
      baseData.gamePlatform = ['PC', 'PlayStation 5', 'Xbox Series X/S', 'Nintendo Switch 2']
      baseData.applicationCategory = 'Game'
      baseData.datePublished = '2026-02-13'
    }
  ...
  const handleSEO = async () => {
    ...
    if (dynamicRouteNames.has(routeName)) {
      try {
        ...
        } else if (routeName === 'news-detail') {
          // 加载 news 数据
          const news = await loadDataForSEO('news')
          const searchId = route.params.id || ''
          const cleanSearchId = searchId.replace(/^\//, '').replace(/\/$/, '')
          
          item = news.find(n => {
            if (!n.addressBar) return false
            const cleanAddressBar = n.addressBar.replace(/^\//, '').replace(/\/$/, '')
            return cleanAddressBar === cleanSearchId
          })
        }
        ...
        if (item) {
          if (item.seo) {
            ...
            finalSEOData = {
              ...finalSEOData,
              title: item.title ? `${item.title} - Reanimal Game` : finalSEOData.title,
